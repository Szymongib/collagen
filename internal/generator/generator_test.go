...
`
	renderedTypeWithPointers = `// Code generated by collagen. DO NOT EDIT
package tests
...
`
	...
	renderedContainsFuncWithPointers = `
// Contains determines if the slice contains the provided element
func (collection Tests) Contains(element *Test) bool {
	for _, e := range collection {
		if e == element {
...
`
	...
	renderedExistsFuncWithPointers = `
// Exist returns if the collection contains the element that satisfy passed function
func (collection Tests) Exist(f func(element *Test) bool) bool {
	for _, e := range collection {
		if f(e) {
...
`
	...
	renderedFilterFuncWithPointers = `
// Filter returns collection of elements that satisfied the check
func (collection Tests) Filter(f func(item *Test) bool) Tests {
	filtered := Tests{}
	for _, e := range collection {
...
func TestGenerateCollectionMethods(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{
			description:    "use pointers is specified",
			structure:      testStruct,
			functions:      []string{},
			usePointers:    true,
			expectedOutput: renderedTypeWithPointers + renderedContainsFuncWithPointers + renderedDropFunc + renderedExistsFuncWithPointers + renderedFilterFuncWithPointers + renderedMapFuncWithPointers + renderedTakeFunc,
		},
	} {
		t.Run("should "+testCase.description, func(t *testing.T) {
			// when
			output, err := GenerateCollectionMethods(testCase.structure, testCase.functions, testCase.usePointers)
